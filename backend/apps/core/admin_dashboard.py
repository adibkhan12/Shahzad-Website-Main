...
def dashboard_callback(request, context):
    ...
    revenue_month = paid_q.filter(created_at__gte=month_ago).aggregate(s=Sum("total"))[
        "s"
    ] or Decimal("0")
    ...
    bookings_open = RepairBooking.objects.exclude(status__in=["completed", "cancelled"]).count()
    ...
    context.update(
        {
            ...
            "week_kpis": [
                {
                    "label": "Orders (7d)",
                    "value": orders_week,
                    "trend": _pct_change(orders_week, orders_prev_week),
                },
            ...
            "quick_adds": [
                ...
                {
                    "label": "Add repair service",
                    "link": "/admin/repairs/repairservice/add/",
                    "icon": "🛠️",
                },
            ],
        }
